...
HOST_HEADERS = {"Authorization": "Bearer tbk_h_vs1b_host"}
WORKER_TOKEN = "tbk_w_vs1b_worker"
...
def test_vs1b_webpage_extraction_end_to_end(tmp_path: Path) -> None:
    db_path = tmp_path / "tokenbank.db"
    runner = CliRunner()
    html = (
        "<html><head><title>Example Research Note</title></head>"
        "<body><main>Private capacity treats webpage content as data.</main></body>"
        "</html>"
    )
    ...
    assert submission["work_unit_id"].startswith("wu_vs1b_")
    ...
    assert submission["backend_class"] == "browser_fetch"
    assert submission["backend_id"] == "backend:webpage_extraction:browser_fetch:v0"
    assert submission["verifier_recipe_id"] == "webpage_extraction_v0"
    ...
    assert summary["task_type"] == "webpage_extraction"
    assert summary["backend_class"] == "browser_fetch"
    ...
    assert output["untrusted_content"] is True
    assert output["fetched"] is False
    assert output["extracted"]["title"] == "Example Research Note"
    assert output["extracted"]["untrusted_content"] is True
    ...
    assert verifier_body["verifier_recipe_id"] == "webpage_extraction_v0"
